...
using QLKARA.App_Code.Class;


namespace QLKARA.App_Code.User
{
	public class KHDatPhong
	{
		...
		{
			List<Phong> dsPhong = new List<Phong>();
			string sql = "Select * From phong,loaiphong where Phong.MaLoaiPhong = loaiphong.MaLoaiPhong ORDER BY MaPhong DESC";
			conn.Open();
			SqlCommand cmd = new SqlCommand(sql, conn);
			SqlDataReader reader = cmd.ExecuteReader();
			while (reader.Read())
		...
		public int ThemDatPhong(dsDatPhong dsDatPhong)
		{
			string sql = "INSERT INTO (HoTen, sdt, thoigianvao, thoigianra,MaPhong) output INSERTED.MaDatPhong Values (@hoten, @sdt, @timeIn, @timeOut,@maphong)";
			conn.Open();
			SqlCommand cmd = new SqlCommand(sql, conn);
